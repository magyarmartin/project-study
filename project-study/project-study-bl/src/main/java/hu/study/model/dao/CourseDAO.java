...
public class CourseDAO extends BasicDAO<Course> {
     ...
     */
    public List<Course> findLikeName( @NonNull final String name, @NonNull String orderType, @NonNull String direction, int limitNum ) {
        ...
        switch( orderType ) {
            ...
            case "ratings":
                query.select( course ).from( course ).leftJoin( course.ratings, rating ).groupBy( course )
                        .where( course.name.lower().like( escapeName.toLowerCase() ) )
                        .orderBy( getOrdering( QRating.rating.score.avg(), direction ) );
                break;
    ...
    private OrderSpecifier<?> getOrdering( ComparableExpressionBase<?> name, String direction ) {
     ...
     */
    @Override
    public void create( @NonNull Course course ) throws IllegalArgumentException {
        if( !isCourseExist( course ) ) {
    ...
    private boolean isCourseExist( @NonNull Course course ) {
